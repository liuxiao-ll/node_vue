      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in addressListFilter" :class="{'check': currentIndex === index}" @click="currentIndex = index; selectedAddrId = item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
            ...
            <div class="shipping-addr-more" v-show="addressList.length > 2">
          ...
          <transition name="fade">
            <div v-show="Add">
              <div class="page-title-normal checkout-title">
                <h2><span>添加新的地址</span></h2>
              </div>
              <div class="shipping-method-wrap">
                <div class="shipping-method">
                  <label for="username">用户名</label><input type="text" v-model="userName">
                  <label for="streetName">收货地址</label><input type="text" v-model="streetName">
                  <label for="postCode">邮编</label><input type="text" v-model="postCode">
                  <label for="tel">电话</label><input type="text" v-model="tel">
                </div>
                <button type="button" class="btn" style="margin-top:15px" @click="addAddress">确定</button>
              </div>
            </div>
          </transition>
      ...
      this.$http.get('/users/addressList').then((res) => {
        ...
        if (res.status === '0') {
          ...
          if (flag === true) {
            ...
            this.selectedAddrId = b[0].addressId
          }
        }
      })
    },
